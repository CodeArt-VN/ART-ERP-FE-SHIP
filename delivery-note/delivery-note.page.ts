...
export class DeliveryNotePage extends PageBase {
            ...
            for (let si = 0; si < this.sheets.length; si++) {
              ...
              for (let i = 0; i < s.Customers.length; i++) {
                ...
                for (let j = 0; j < c.Orders.length; j++) {
                  ...
                  o.OriginalTotalAfterDiscountFromSalesmanText = lib.currencyFormat(
                    o.OriginalTotalAfterTax - o.OriginalDiscountFromSalesman,
                  );
  ...
  calcSheet1(o) {
    ...
    o.OrderLines.sort(
      (a, b) => parseFloat(a.ItemSort) - parseFloat(b.ItemSort) || a.Item.Code.localeCompare(b.Item.Code),
    );
  ...
  calcSumQty(findItem, l) {
    ...
    if (l.Item.UoM.Id == l.Item.BaseUoM.Id || l.Item.UoM.Id == l.Item.InventoryUoM.Id) {
      ...
      let qdThung = parseInt(
        ((tongLeQty * l.Item.InventoryUoM.AlternativeQuantity) / l.Item.InventoryUoM.BaseQuantity).toString(),
      );

      if (qdThung > 0) {
        calcUom.InventoryQuantity += qdThung;
        let leConLai =
          tongLeQty - (qdThung * l.Item.InventoryUoM.BaseQuantity) / l.Item.InventoryUoM.AlternativeQuantity;
  ...
  calcSheetBuGia(o) {
    ...
    o.OrderLines.forEach((l) => {
      let findItem = itemList.find(
        (d) =>
          d.IDItem == l.IDItem && d.IDUoM == l.Item.UoM.Id && Math.abs(d.SalesmanPrice * 10 - l.SalesmanPrice * 10) < 1,
      );
      ...
      findItem.OriginalTotalAfterDiscountFromSalesmanText = lib.currencyFormat(
        findItem.OriginalTotalAfterDiscountFromSalesman,
      );
