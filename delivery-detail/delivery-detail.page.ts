...
export class DeliveryDetailPage extends PageBase {
  ...
  loadedData(event) {
    if (this.item && this.item.SaleOrder && this.item.SaleOrder.OrderLines) {
      ...
      this.item.SaleOrder.OrderLines.sort(
        (a, b) => parseFloat(a.ItemSort) - parseFloat(b.ItemSort) || a.ItemCode.localeCompare(b.ItemCode),
      );
      ...
      if (this.checkIsEditableShipmentDetail()) {
        ...
        if (this.item.IsDebt && this.checkIsEditableShipmentDebt()) {
          this.setDaNhan(
            '=',
            this.item.Debt -
              (this.item.SaleOrder.DiscountFromSalesman - this.item.SaleOrder.ReceivedDiscountFromSalesman),
          );
        }
      }
  ...
  renderCurrencyFormat() {
    ...
    if (this.item.IsDebt) {
      this.item.MaxReceive =
        this.item.Debt - (this.item.SaleOrder.DiscountFromSalesman - this.item.SaleOrder.ReceivedDiscountFromSalesman);
      this.item.ReceivedPercent = !this.item.MaxReceive
        ? 0
        : Math.round((this.item.Received / this.item.MaxReceive) * 100);
    } else {
      this.item.MaxReceive =
        this.item.SaleOrder.TotalAfterTax -
        (this.item.SaleOrder.DiscountFromSalesman - this.item.SaleOrder.ReceivedDiscountFromSalesman);
      this.item.ReceivedPercent = !this.item.MaxReceive
        ? 0
        : Math.round((this.item.Received / this.item.MaxReceive) * 100);
  ...
  changeDaNhan(e = null) {
    ...
    if (this.item.IsDebt) {
      ...
      this.item.SaleOrder.CustomerDebt =
        this.item.SaleOrder.Debt -
        (this.item.SaleOrder.DiscountFromSalesman - this.item.SaleOrder.ReceivedDiscountFromSalesman);
    }
  ...
  changeDiscount(i, e, ck) {
    ...
    let sumCK3 = this.item.SaleOrder.OrderLines.map((i) => i.DiscountFromSalesman).reduce(
  ...
  UpdatePosition() {
    let apiPath = {
      method: 'PUT',
      url: function () {
        return ApiSetting.apiDomain('SHIP/Shipment/DeliveryUpdatePosition');
      },
    };
